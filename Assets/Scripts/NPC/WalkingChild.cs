...
public class WalkingChild : NPCParent.IState
{
    ...
    {
        this.npc = npc;
        susSuund = npc.sussyBoolList[NPCParent.DirectionIndex];
        susAnima = npc.sussyBoolList[NPCParent.BounceIndex];
    }
    ...
    public void Enter()
    {
        ...
        if (susSuund) direction = 1.5f * SnapToCardinal(direction);
        
        Animator animator = npc.GetComponent<Animator>();
        if (susAnima == false) animator.SetTrigger("Walk");
        else animator.SetTrigger("Bounce");
